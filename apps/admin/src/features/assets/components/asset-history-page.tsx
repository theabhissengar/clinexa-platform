...
import { getAssetHistory } from "@/features/assets/api/assets-api";
import type { AssetHistoryRow } from "@/features/assets/types";

export function AssetHistoryPage() {
  const params = useParams<{ id: string }>();
  const [rows, setRows] = useState<AssetHistoryRow[]>([]);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    void getAssetHistory(params.id)
      .then(setRows)
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-10">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Entity field and lifecycle change history. Not the platform Audit Log.
        </p>
      </div>
