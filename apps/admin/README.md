...
- Feature-page `emerald-*` / `amber-*` classes remain until remaining modules are adopted in later 5F follow-ups (Plans/Roles/Products).
...
- **Phase 5C / shared UX patterns:** reusable presentation components live in `src/components/patterns/` (`ClinexaPage`, `PageHeader`, `PageBody`, `DataTable`, `FilterBar`, empty/error/skeleton states, `ConfirmDialog`, `FormSection`, `FieldGrid`, `EntityDetailHeader`, `DetailSection`). Users/Orders/Subscriptions (CRM+Guardian) adopt these in Phase 5F. `StatusBadge` stays in `src/components/ui/status-badge.tsx`. Search/pagination implementations live in patterns with compatibility re-exports from previous feature paths. Mutation feedback convention: success → toast or semantic `text-success`; recoverable failure → toast and/or inline; blocking validation/API errors → inline. Do not use `richColors`. Unlisted preview: `/dev/design-system` (not in `nav-config`, not a product destination).
- **Phase 5E / dashboards + shell visuals:** `/crm` and `/guardian` are permission-aware dashboards composed from existing list/count APIs only. `PageCanvas` reuses the finalized Login form-column background treatment, while `MetricCard` composes the shared solid `Card`. The shared sidebar and header use restrained translucent semantic surfaces and backdrop blur; navigation, permissions, routes, collapse behavior, application switching, theme state, breadcrumbs, and the 1024px breakpoint are unchanged. No charts, fake metrics, new packages, or Login changes.
- **Phase 5F / core module UI:** CRM and Guardian Users, Orders, and Subscriptions list/detail screens use Phase 5C patterns and `StatusBadge` product labels. Guardian Class D reason collection uses controlled Dialog+Input (not `ConfirmDialog`). No API, RBAC, route, or domain-logic changes.
