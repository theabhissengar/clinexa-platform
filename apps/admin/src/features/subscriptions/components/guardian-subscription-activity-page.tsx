...
import { listAdminSubscriptionActivity } from "@/features/subscriptions/api/admin-subscriptions-api";
import {
  formatDateTime,
  getErrorMessage,
...
export function GuardianSubscriptionActivityPage() {
  ...
  useEffect(() => {
    void listAdminSubscriptionActivity(params.id)
      .then(setRows)
      .catch((err) =>
  ...
  return (
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-4 px-4 py-8 md:px-6">
      <Link
        href={`/guardian/subscriptions/${params.id}`}
